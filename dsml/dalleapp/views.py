...
from .serializers import ConfigurationSerializer, TextSerializer
...
def getSettings():
    settings=Configuration.objects.first()
    if(settings is None):
        settings = Configuration(
            summaryModel=get('SUMMARIZATION_MODEL',""),
            openaiSessK=get('OPENAI_SESS_K',""),
            nImages=int(get('OPENAI_N_IMAGE',4))
        )
...
class Dalle2ApiView(APIView):
    def get(self, request, id=None):
        text = request.query_params.get("text")
        dalle = Dalle2(settings.openaiSessK) 
        generations = dalle.generate_amount(text,settings.nImages)
...
class SettingsApiView(APIView):
    ...
    def post(self, request, id=None):
        s=Configuration.objects.first()
        print(request.data["openaiSessK"])
        serializer = ConfigurationSerializer(s,data=request.data,partial=True)
        if serializer.is_valid():
            n=serializer.save()
            global settings
            settings=n
